...
document.querySelector('.buttons').onclick = (event) => 
{
    ...
    else if (key == '=') 
    {
        ...
        {
            out.textContent = calculated.toFixed(6); logger = [calculated.toFixed(6)];
        }
        else 
        {
            out.textContent = calculated; logger = [calculated];
        }
    }
    else if (key in action) 
    {
        if (squenceDigits != '') logger.push(squenceDigits); squenceDigits = String();
